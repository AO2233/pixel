...
Pixel includes several open-source libraries. Thanks to their authors and contributors.

Pixel 使用了以下开源组件，感谢这些项目的作者与贡献者。

| Component | Version | Used for | License | Upstream |
| --- | ---: | --- | --- | --- |
...
These components are not relicensed under Pixel's license. Each component remains subject to its upstream license and copyright notice. When redistributing the theme, keep this file and any notices retained in the source files.

以上组件不会被 Pixel 重新许可，仍分别适用各自的上游许可证与版权声明。再分发主题时，请保留本文件以及源码中已有的声明。

KaTeX's bundled math fonts are licensed separately under the SIL Open Font License 1.1. They are different from the theme fonts in `static/fonts/`, which are covered by the separate [font notice](FONT_NOTICE.md).

KaTeX 附带的数学字体单独采用 SIL Open Font License 1.1，与 `static/fonts/` 中的主题字体不是同一授权范围；后者请参阅[字体声明](FONT_NOTICE.md)。
